...
function StatCard({ label, value, icon: Icon, color, sub }: StatCardProps) {
  return (
    <div className="rounded-2xl p-5 relative overflow-hidden transition-all hover:-translate-y-0.5"
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        <StatCard label="Receita Bruta" value={formatCurrency(totals.gross)} icon={DollarSign} color="#6366F1" sub={`${totals.orders} pedidos`} />
        <StatCard label="Receita Líquida" value={formatCurrency(totals.net)} icon={TrendingUp} color="#10B981" sub={`Margem ${margin}%`} />
        <StatCard label="Descontos" value={formatCurrency(totals.discount)} icon={TrendingDown} color="#EF4444" sub="Total de desconto" />
        <StatCard label="Ticket Médio" value={formatCurrency(avgTicket)} icon={BarChart3} color="#F59E0B" sub={`${totals.orders} pedidos`} />
      </div>
