...
import {
  ...
  Phone, RefreshCw, ShieldCheck, XCircle,
} from 'lucide-react';
    ...
    <div className="space-y-5">
      ...
      <Card className="p-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <div className="rounded-xl p-4 col-span-2" style={{ background: 'var(--input-bg)', border: '1px solid var(--border)' }}>
            ...
            {license?.last_renewed_at && (
              <p className="text-[11px] mt-0.5" style={{ color: 'var(--text-muted)' }}>
                Último resgate: {formatDate(license.last_renewed_at)}
              </p>
            )}
          </div>
        </div>
        ...
        {isActive && daysLeft <= 7 && daysLeft > 0 && (
          <div className="flex items-center gap-3 mt-4 px-4 py-3 rounded-xl"
            ...
            <p className="text-xs font-semibold" style={{ color: COLORS.warningLight }}>
              Seu plano expira em <strong>{daysLeft} dia{daysLeft !== 1 ? 's' : ''}</strong>. Resgate um código para renovar.
            </p>
          </div>
        )}
        ...
        {isGrace && (
          <div className="flex items-center gap-3 mt-4 px-4 py-3 rounded-xl"
            ...
            <p className="text-xs font-semibold" style={{ color: COLORS.dangerLight }}>
              Plano expirado. Período de graça ativo: <strong>{daysLeft} dia{daysLeft !== 1 ? 's' : ''}</strong> restante{daysLeft !== 1 ? 's' : ''}.
            </p>
          </div>
        )}
      </Card>
      ...
      {/* ── Resgatar código ── */}
      <Card className="p-6">
        <SectionHeader icon={Key} label="Resgatar Código" subtitle="Ative ou renove seu plano" color={COLORS.accent} />
        ...
        <div className="space-y-3">
          ...
          <p className="text-[11px]" style={{ color: 'var(--text-muted)' }}>
            Os dias são <strong style={{ color: 'var(--text-secondary)' }}>acumulados</strong> ao saldo atual se seu plano ainda estiver ativo.
          </p>
        </div>
      </Card>
